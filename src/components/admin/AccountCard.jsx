...
export default function AccountCard({
  ...
  return (
    <div
      className={`glass-panel group relative flex flex-col overflow-hidden rounded-[2rem] border p-6 transition-all shadow-sm ${isCurrentTurn
    ...
    >
      ...
      {isMarketing && (
        <div className="absolute left-6 top-6 z-10 flex flex-col items-center gap-1">
          <span className={`flex h-8 w-8 items-center justify-center rounded-full font-bold backdrop-blur-sm border ${isCurrentTurn
      ...
      <div className={`flex items-center gap-4 ${isMarketing ? 'ml-12' : ''}`}>
        <img
          src={account.photo ? `https://api.glorymaspro.biz.id/storage/${account.photo}` : `https://ui-avatars.com/api/?name=${encodeURIComponent(account.name)}&background=D4AF37&color=fff`}
          alt={account.name}
        ...
        <div>
          ...
          <span className={`mt-1 inline-block rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest ${account.role?.toLowerCase() === 'admin'
